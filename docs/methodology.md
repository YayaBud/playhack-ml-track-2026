# Methodology — PlayHack ML Track 2026 (deep dive)
...
```
.
├── README.md                       ← Short overview (start there)
├── RUNME.md                        ← How to run the submission ZIP
├── RESULTS.md                      ← Scores, per-model tables, SOTA benchmark
├── predict.py                      ← Inference entry point: raw CSVs → submission
├── make_zip.py                     ← Builds the Round-1 deliverable archive
├── requirements.txt                ← Pinned dependencies
├── submission.csv                  ← Final predictions for 1,100 test athletes
│
├── docs/
│   ├── problem_statement.md        ← Competition brief + the official metric (PDF p.6)
│   ├── findings.md                 ← Numbered findings, incl. the metric analysis
│   └── methodology.md              ← This document
│
├── models/                         ← Persisted models (2.2 MB), written by build_final_models.py
│   ├── manifest.json               ← Feature lists, blend weights, threshold, versions
│   ├── clf_{lgb,xgb,cat}.joblib    ← Injury classifier ensemble
│   ├── onset_{lgb,xgb}.joblib      ← Onset-day regressors
│   ├── recovery_{lgb,xgb,cat}.joblib
│   └── recovery_grp.joblib         ← Cross-fitted sport+gender median table
│
├── reports/                        ← Figures, logs, metric files
│   ├── 01..06_*.png                ← EDA figures (split, labels, rates, drivers, workload, correlations)
│   ├── metrics.json                ← All final numeric results
│   ├── oracle.json                 ← Bayes ceiling values
│   ├── feature_sweep.json          ← Feature-count sweep
│   ├── bench_autogluon.json        ← AutoGluon benchmark scores
│   ├── *.npy                       ← OOF / test prediction arrays
│   └── *.log                       ← Run logs
│
└── src/
    ├── features.py                 ← Leak-safe feature engineering (223 features)
    ├── score.py                    ← The organisers' official metric (PDF p.6)
    ├── rethreshold.py              ← Threshold selection against that metric
    ├── build_final_models.py       ← Refit on all data, persist models/
    ├── final.py                    ← 3-family ensemble + OOF blender
    ├── recovery_v2.py              ← Adds the group-median recovery candidate
    ...
    ├── eda.py                      ← EDA figures for the deck
    ├── test_leakage.py             ← Automated leakage tests
    ├── report.py                   ← Renders RESULTS.md from metrics.json
    ├── validate_submission.py      ← Submission schema & boundary checks
    ├── pipeline.py, train.py       ← Superseded early pipelines (kept for history)
...
The whole inference path is one command — it rebuilds features from raw CSVs,
loads every persisted model, blends them with the stored weights and applies the
stored threshold:

```bash
python predict.py --data-root data --split test --out submission.csv
```

To drive the artifacts yourself, go through the manifest rather than hardcoding
filenames — it carries the per-head feature lists, blend weights, the category
level ordering XGBoost's codes depend on, and the library versions the pickles
were written under:
...
head = man["heads"]["clf"]           # also "onset", "recovery"
print(head["weights"])               # {'lgb': 0.002, 'xgb': 0.546, 'cat': 0.452}
print(len(head["features"]), "features")

art = joblib.load("models/clf_xgb.joblib")
model, feats = art["model"], art["features"]
```

> Earlier revisions of this file pointed at `models/classifier.txt`, an artifact
> from the superseded `src/pipeline.py`. Those files are gone — they reproduced
> the old pipeline's numbers, not the reported ones.
